...
<body id="page-top">
                    ...
                    <form id="form" class="form-horizontal" method="POST" action="{{route('registration_create')}}">
                      ...
                      <fieldset>
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-sm-9">
                            <input type="email" class="form-control" id="email" name="email" value="{{old('email')}}" placeholder="Use valid email address here" required>
                          </div>
                        </div>
                      </fieldset>
                        ...
                        <div @if ($key == 0) id="dependant_list" @endif class="row form-group">
                          <div class="col-sm-3">
                            <select class="form-control" name="dependant_relationship[]">
                              ...
                              <option value="Spouse" @if (old('dependant_relationship'.".".$key) == 'Spouse') selected="selected" @endif>Spouse</option>
                              <option value="Kids"@if (old('dependant_relationship'.".".$key) == 'Kids') selected="selected" @endif>Kids</option>
                              <option value="Infant"@if (old('dependant_relationship'.".".$key) == 'Infant') selected="selected" @endif>Infant</option>
                              <option value="Others"@if (old('dependant_relationship'.".".$key) == 'Others') selected="selected" @endif>Others</option>
                            </select>
                          </div>
                          <div class="col-sm-7">
                            <input type="text" class="form-control" name="dependant_name[]" placeholder="Name" value="{{old('dependant_name'.".".$key)}}">
                          </div>
                          <div class="col-sm-2">
                            <input type="number" class="form-control" name="dependant_age[]" placeholder="Age" min="1" max="80" value="{{old('dependant_age'.".".$key)}}">
                          </div>
                        </div>
                        @endforeach
                        @else
          ...
          <div class="row">
            <div class="col-md-10 col-md-offset-1 call-to-action">
                ...
                <ol>
                  ...
                  <li>Children age <em>3-10 years old</em> or with the <em>height of 90-130 cm</em> are categorized as <mark>Kids</mark>.</li>
                  <li>Children <em>below 3 years old</em> or with <em>height less than 90 cm</em> are categorized as <mark>Infant</mark>.</li>
                  ...
                  <li>Every admission to the Family Day will be charged for <mark>RM10.00 per registration</mark> as commitment fee.</li>
                  ...
                  <li>Registration and payment should be made before <mark>{{ config('app.paymentday') }}</mark>.</li>
                  <li>For TM staff who are husband and wife, please register in two separate forms. Please make sure your children information are only in one of the form.</li>
                ...
                <table class="table table-condensed text-center">
                  ...
                  <tbody>
                    <tr title="{{\Carbon\Carbon::create(2017,9,27)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,9,27)->diffInDays(\Carbon\Carbon::now('Asia/Kuala_Lumpur'),false) > 0) class="passed" @endif>
                      <td>Menara TM</td>
                      <td>27 September 2017</td>
                    </tr>
                    <tr title="{{\Carbon\Carbon::create(2017,9,28)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,9,28)->diffInDays(\Carbon\Carbon::now('Asia/Kuala_Lumpur'),false) > 0) class="passed" @endif>
                      <td>Menara TM</td>
                      <td>28 September 2017</td>
                    </tr>
                    <tr title="{{\Carbon\Carbon::create(2017,10,4)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,10,4)->diffInDays(\Carbon\Carbon::now(),false) > 0) class="passed" @endif>
                      <td>TM Annexe 2</td>
                      <td>4 October 2017</td>
                    </tr>
                    <tr title="{{\Carbon\Carbon::create(2017,10,5)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,10,5)->diffInDays(\Carbon\Carbon::now(),false) > 0) class="passed" @endif>
                      <td>TM Annexe 2</td>
                      <td>5 October 2017</td>
                    </tr>
                    <tr title="{{\Carbon\Carbon::create(2017,10,9)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,10,9)->diffInDays(\Carbon\Carbon::now(),false) > 0) class="passed" @endif>
                      <td>Menara TM</td>
                      <td >9 October 2017</td>
                    </tr>
                    <tr title="{{\Carbon\Carbon::create(2017,10,10)->diffForHumans()}}" @if (\Carbon\Carbon::create(2017,10,10)->diffInDays(\Carbon\Carbon::now(),false) > 0) class="passed" @endif>
                      <td>Menara TM</td>
    ...
    <section class="map" id="location">
        ...
        <iframe width="100%" height="100%" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyBD6fzCA2JRXb2ZxJROiHkP6jVih1rMH00&q=Bangi+Wonderland"></iframe>
        <br />
        <small>
            <a href="https://www.google.com/maps/embed/v1/place?key=AIzaSyBD6fzCA2JRXb2ZxJROiHkP6jVih1rMH00&q=Bangi+Wonderland"></a>
        </small>
        </iframe>
    </section>
